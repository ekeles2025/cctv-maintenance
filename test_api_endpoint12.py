    ...
    with app.test_client() as client:
        ...
        if response.status_code == 200:
            ...
            if redirect_response.status_code == 200:
                ...
                # Extract CSRF token
                import re
                csrf_match = re.search(r'name="csrf_token" type="hidden" value="([^"]+)"', login_data)
                if csrf_match:
                    ...
                    print('Looking for alternative CSRF patterns...')
                    # Try different patterns
                    patterns = [
                        r'name="csrf_token" type="hidden" value="([^"]+)"',
                        r'csrf_token.*?value="([^"]+)"',
                        r'"csrf_token":\s*"([^"]+)"',
                        r'csrf_token.*?content="([^"]+)"'
                    ]
                    for pattern in patterns:
                        match = re.search(pattern, login_data)
                        if match:
                            print(f'Found CSRF with pattern: {pattern}')
                            print('CSRF token:', match.group(1)[:20] + '...')
                            break
                    else:
                        print('No CSRF token found with any pattern')
                        print('Login page preview (looking for CSRF):')
